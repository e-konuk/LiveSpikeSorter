...
def calvin_compute_cluster_centroids(spike_clusters, spike_positions):
    """
    Compute the centroid (mean x and y positions) of each cluster.

    Parameters:
    - spike_clusters (np.ndarray): 1D array of cluster indices for each spike (shape: [num_spikes]).
    - spike_positions (np.ndarray): 2D array of x and y positions for each spike (shape: [num_spikes, 2]).

    Returns:
    - cluster_centroids (dict): Dictionary mapping cluster indices to their centroid positions (tuple of x and y).
    """
    unique_clusters = np.unique(spike_clusters)
    cluster_centroids = {}

    for cluster in unique_clusters:
        # Get a boolean mask for spikes belonging to the current cluster
        mask = spike_clusters == cluster
        # Select the positions of spikes in the current cluster
        cluster_positions = spike_positions[mask]
        # Compute the mean x and y positions
        centroid = cluster_positions.mean(axis=0)
        # Store the centroid in the dictionary
        cluster_centroids[cluster] = (centroid[0], centroid[1])
...
def save_to_phy(st, clu, tF, Wall, probe, ops, imin, results_dir=None,
    ...
    cluster_centroids = calvin_compute_cluster_centroids(spike_clusters, spike_positions)
    ...
    np.save((results_dir / 'cluster_centroids.npy'), cluster_centroids)
